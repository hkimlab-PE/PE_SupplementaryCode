...
# Test Model
def Model_Inference(sess, TEST_X, model, args, load_episode, test_data_num, testvalbook, testvalsheet, col_index=1, TEST_mod_X=None, TEST_bio=None):
    test_batch = 500
    test_spearman = 0.0
...
def getfile_inference(filenum):
    ...
    for l in range(17, data_n):
        try:
            ...
            mod_seq.append(data_split[1])
            tmp_bio = []
            for idx in range(2, 22):
                tmp_bio.append(np.array(data_split[idx]))
            bio.append(tmp_bio)
        except:
            print(data[l])
    #loop end: l
...
for TEST_NUM_index in range(len(TEST_NUM_SET)):
    ...
    tmp_X, pre_X, tmp_mod_X, pre_mod_X, bio = getfile_inference(TEST_NUM)
    TEST_X.append(tmp_X)
...
for index in range(len(best_model_list)):
    ...
    with tf.Session(config=conf) as sess:
        ...
        Model_Inference(sess, TEST_X[0], model, args, load_episode, length, testbook, testsheet[0], TEST_mod_X=TEST_mod_X[0], TEST_bio=TEST_bio[0])
        testbook.close()
